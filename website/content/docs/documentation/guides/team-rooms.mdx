...
You will need an MQTT 5 broker. The recommended path is a free managed one: [A team room on HiveMQ Cloud](./hivemq-cloud.mdx) walks it end to end with screenshots. EMQX Cloud works too, and so does your own mosquitto, below.
...
Create the account the team shares, at the same path `password_file` names:
...
# macOS
mosquitto_passwd -c "$(brew --prefix)"/etc/mosquitto/passwd team
```
...
On the mini, install mosquitto, save the configuration above at `"$(brew --prefix)"/etc/mosquitto/mosquitto.conf`, and only then start the service:

```bash
brew install mosquitto
# save mosquitto.conf now, the service reads it once at startup
brew services start mosquitto
```

If the service was already running when you saved the file, `brew services restart mosquitto` picks it up. A CI box is normally already set to never sleep, which is the other thing a broker needs.
...
* No open ports at all: a Cloudflare Tunnel in front of a mosquitto WebSocket listener, joined as `wss://team@broker.example.com`. The minimal configuration above does not include that listener, so add `listener 8083 localhost` with `protocol websockets` on the next line, and point the tunnel's origin at `http://localhost:8083`. The tunnel connects from the same machine, so that listener stays loopback-only and plaintext never crosses the network. The tunnel's edge certificate is publicly trusted, so the system-store rule is satisfied. This route is supported but has not yet been exercised by a real team, so try it before the team depends on it.
