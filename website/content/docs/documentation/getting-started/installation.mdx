...
* An iOS simulator, if you want the docked presentation. That means Xcode's Simulator.app (up to Xcode 26) or Device Hub (Xcode 27 and later) with at least one iOS runtime. Notchboard docks to either. The app is fully usable without one, as an undocked panel opened from the menu bar.
* The Android emulator docks too, with one prerequisite: it must run in its own window, not embedded in Android Studio's Running Devices tool window. Untick Settings → Tools → Emulator → "Launch in the Running Devices tool window", or start it with `emulator -avd <Name>`. The deeplink bridge needs `adb` from Android's platform-tools.
...
Notchboard checks GitHub for a newer release once a day. When one exists, a small dot appears on the menu-bar icon and Settings shows the version under Updates. Nothing pops up. Install it from the menu bar or from Settings when it suits you, or from Homebrew:

```bash
brew upgrade --cask thepearl/tap/notchboard
```

Either route keeps the Accessibility grant, because every release carries the same signing identity. A copy built from source never checks for updates, and loses the grant on each rebuild, so you grant it again.
...
You need macOS 14 or later and Xcode 26 or later. An older Xcode fails loudly, on the first implicitly main-actor initialiser. The project has two Swift package dependencies, [mqtt-nio](https://github.com/swift-server-community/mqtt-nio) and [Sparkle](https://github.com/sparkle-project/Sparkle), fetched on the first build, so later builds work offline.
...
<Card href="./quickstart.mdx" title="Quickstart" />
