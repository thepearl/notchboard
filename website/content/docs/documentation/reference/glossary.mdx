...
**Secret field.** A field typed `secret`. Its values move out of every plaintext surface, into the Keychain on disk and into an encrypted envelope in an export file. See [field types and secrets](../core-concepts/field-types-and-secrets.mdx).
