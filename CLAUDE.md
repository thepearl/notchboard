...
**Nothing renders a secret-typed value except the detail view's reveal toggle** (2026-08-13, vision.md §13.16). Every on-screen read of an element value goes through `NBGroup.displayValue(_:of:)`, which masks secret-typed fields with the single `NBGroup.secretMask` constant — a fixed ten bullets, since a mask sized to its value leaks the length. This is load-bearing for the list: a row's subtitle comes from `secondaryKey`, which is always the group's *first* field, so a user-defined group leading with a secret printed it in cleartext on every row until this landed. `secondaryText`'s group-id special cases name their fields directly and must keep going through the accessor too. **Search deliberately excludes secret-typed values** (same decision): with the subtitle masked, a match on a password produced a row with nothing on screen containing the query, and a field that confirms substrings of a secret is a guessing oracle on a panel that sits open beside a shared Simulator. The haystack is keyed off the schema, not off `values`. `SecretMaskingTests` fails if any of that regresses.
...
- `secondaryText` still special-cases `group.id == "promos"`/`"products"` for row subtitles (guarded on the fields existing). Known debt; the login button was made schema-driven. Leak-safe since 2026-08-13 (every branch masks secrets) but not fixed.
